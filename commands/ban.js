...
module.exports = {
  ...
  if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send(":no_entry_sign: **Error:** No tienes **Ban Members** permisos!");
  if (message.mentions.users.size < 1) return message.channel.send('Tienes que mencionar a alguien.').catch(console.error);
  ...
  if  (!logchannel){
    ...
    message.channel.send(`:hammer:  Hecho. You don't have to worry about that shit head anymore, I have banned them! Also I\'ve logged the ban in <#${logchannel.id}>.`)
  ...
  help: {
    ...
    category: '',
    guildOnly: true,
    description: 'Banea el usuario mencionado.',
    usage: 'ban [mension] [razon]'
  }
}